    ...
    public class ResExtension : MarkupExtension
    {
        ...
        public ResExtension()
        {
            ...
            if (LocalizationSettings.Current.CheckForCultureChange)
                LocalizationSettings.Current.CultureChanged += this.UpdateTarget;
        }
        ~ResExtension()
        {
            LocalizationSettings.Current.CultureChanged -= this.UpdateTarget;
        ...
        public override object ProvideValue(IServiceProvider serviceProvider)
        {
            ...
            if (this.targetObject == null)
            {
                ...
                this.targetObject = targetService.TargetObject as DependencyObject;
                this.targetProperty = targetService.TargetProperty as DependencyProperty;
                this.typeConverter = TypeDescriptor.GetConverter(this.targetProperty.PropertyType);
        ...
        private object ProvideValueInternal()
        {
            ...
            else
            {
                try
                {
                    ...
                    if (this.propertyInfo == null)
                    {
                        ...
                        IXamlTypeResolver service = this.serviceProvider.GetService(typeof(IXamlTypeResolver))
                                                    as IXamlTypeResolver;
                        Type memberType = service.Resolve(typeName);

                        string propName = this.Static.Substring(index + 1);
                        this.propertyInfo = memberType.GetProperty(propName,
                                            BindingFlags.Public | BindingFlags.Static |
                                            BindingFlags.FlattenHierarchy);
                    }
                    localized = this.propertyInfo.GetValue(null, null);
                }
                catch
                {
            ...
            try
            {
                ...
                if (localized != null &&
                    ...
                    this.typeConverter.CanConvertFrom(localized.GetType()))
                    localized = this.typeConverter.ConvertFrom(localized);

                // Apply a type converter if one was specified
                if (this.Converter != null)
                    localized = this.Converter.Convert(localized, this.targetProperty.PropertyType,
            ...
            if (localized == null)
            {
                if (this.targetProperty != null &&
                    this.targetProperty.PropertyType == typeof(string))
                    // Return the key surrounded by question marks for string type properties
                    localized = string.Concat("?", this.Id, "?");
                else
        ...
        internal bool FindDefaultResourceAssembly()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    FrameworkElement element = this.targetObject as FrameworkElement;
                    if (element != null)
                    {
